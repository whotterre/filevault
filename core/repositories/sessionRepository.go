...
type SessionRepository interface {
	...
	DeleteSessionByEmail(ctx context.Context, email string) (bool, error)
}
...
func (r *sessionRepository) DeleteSessionByEmail(ctx context.Context, email string) (bool, error) {
	delCount, err := r.client.Del(ctx, email).Result()
	if err != nil {
		return false, err
	}
	return delCount > 0, nil
